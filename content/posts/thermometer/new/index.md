...
2. 校准效果不理想
    ...
    在校准时，最好采用曲线拟合的方式进行校准，因为红外温度传感器的误差函数为曲线，而非直线。但是在实际校准过程中，受限于红外测温设备上的计算能力，通常不采用曲线拟合的方式，而是采用直线近似拟合。这就导致了校准只适用于比较小的测量范围，如果策略范围较大，校准效果就会不理想。

为了解决传统校准方式的缺点，我们开发了新一代的校准方案。在这个方案中，我们开发了专业的校准软件，将校准数据的计算由红外测温设备转移到计算机上，充分利用了计算机强大的计算能力。这个方案的优点：

1. 一致性高

    在这个方案中，由于校准点的测量结果保存在校准软件中，而不是模块内部，所以校准点的数量和读取次数不受限制。所以这个方案可以通过过采样和最小二乘法对校准数据进行处理，从而能够有效地过滤掉异常数据和噪声，提高了一致性。

2. 理想的校准效果

    这个方案抛弃了原始的线性校准方式，引入了向量校准。校准所需要的向量由校准软件通过Tensorflow计算出。由于向量校准可以拟合曲线，所以这种校准方式能在更大的范围对误差进行补偿，校准结果也就更加理想。

校准系统的详细介绍在[这里](/posts/thermometer/cali/)。

#### 高精度ADC

再一个影响红外测温精度的方面是ADC（模数转换）的选择。由于红外温度传感器输出电压的变化非常的小，每0.01℃的变化对应的电压变化仅为0.002毫伏，所以ADC的选择对分辨率很重要。我们在此选用了业内知名的24位ADC，它能够达到0.0005毫伏的分辨率。

由于AI算法、新一代校准和高精度ADC三个方面的改进，我们最终实现了宽温域，高精度的红外测温模块。
...
高温红外测温模块适用于0-600°C的测温场景。它的测量误差为0.5℃，在精度上也远超[市场上绝大多数红外测温设备](/posts/thermometer/new/images/1.jpg)。

#### 高度可定制

在两款标品之外，我们还可以针对客户的需求进行定制，例如更换特定的红外镜头、指定特定的测温范围和精度。在定制产品中，我们最高的精度可以达到0.01℃。
